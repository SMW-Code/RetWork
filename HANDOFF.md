...
- **build 259**: 로그인 화면에서 이메일 직접 로그인/회원가입 영역 숨김(`display:none`, 코드 보존).
  소셜(Google/Apple/LINE)만 노출. 되살리려면 `#auth-manual-block` + 탭 div의 display 복원.
  → 부수효과: 가입에 Google/LINE 계정 필수 → 대량 가짜계정 어뷰징 자연 차단(특히 LINE=전화번호).
- **build 260**: 추천 보상 텍스트 정정. **실제 동작 = 추천인에게만 +200치리** (`referral.sql`의
  `redeem_referral`은 `v_referrer`에게만 지급, 피추천인 보상 없음). 기존 UI가 "둘 다 200"으로
  잘못 표기돼 있던 것 수정(설정창 카드 + 공유 메시지). 자기추천은 계정 단위(`v_referrer=v_new_user`)로
  차단 중 — 단, 다른 이메일로 만든 2번째 계정 자기추천은 미차단(보상 16엔이라 현재는 허용).
  - 추후 강화 옵션(미적용): 활동 기반 게이트(피추천인 영수증 스캔 후 지급), 추천인당 보상 상한.
