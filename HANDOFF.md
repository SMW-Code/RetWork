...
>
> **버그 수정:** b548 `_cpFromMap` 누수 / b549 영수증 합계 WYSIWYG / **b554 상품교환 원자성 RPC `client_redeem_item`** / **b555 price_pins 중복방지(유니크+upsert)**.
>
> **#6 가게 식별 재아키텍처 (동명 지점 분리) — ✅ 본체 완료:** 상세는 [`NEXT_TASK_place_id_phase2.md`](NEXT_TASK_place_id_phase2.md)
> - 문제: 가게가 `stores.name`(UNIQUE)으로 식별돼 동명 다른 지점이 한 행으로 합쳐짐 + 커뮤니티(댓글/메뉴/사진/별점)가 **store_name 문자열** 기준.
> - 해결: 커뮤니티에 `store_id` 컬럼+dual-write+backfill(2a~2c) → 읽기/맵/persist/어드민 store_id화(2d·2e-1~3d) → **`stores.name UNIQUE 제거(비가역, 검증 0행)**.
> - 식별자 전환: `_ensureStore`(공개=geocode·place_id 우선) / `_storeByNameOrCreate`(persist·수동핀=좌표 그대로, geocode無) / `_storeOr`(읽기 store_id 단독) / `_resolveStoreId`(limit1).
> - **효과:** 앞으로 등록되는 동명 다른 지점은 **다른 place_id → 별도 store_id 행**으로 자동 분리. **기존 합쳐진 데이터는 소급 분리 X.**
> - 백업 테이블: `_bak2c_*`(커뮤니티) · `_bak2e_stores`(stores). 안정화 후 DROP 가능. 기존 stores 는 `place_id` 대부분 NULL(생성당시 미수집) → 활동하면 채워짐.
> - ⚠️ 교훈: 핀 push~첫 `ctRenderPricePins()` **사이에 awaited 네트워크 호출 금지**(수동핀 회귀 원인 b561→567). 수동핀은 **유저가 찍은 좌표가 정답**(geocode 이동 금지).
>
> **버그헌트 (병렬 에이전트 발굴→검증→수정, b571~573):** #1 어드민 가게삭제 자식 store_id화(데이터손실 방지) · #2 공개 stuck-lock(.catch) · #3 공개 geocode 좌표 store_id · #4 어드민 핀 가격필터(minPrice/maxPrice) · #5 user 구독/탈퇴 탭 select · #6 핀 코멘트필터+검색 · #7 featured/대표사진 by-name 리셋 store_id · #8 맵 dedup store_id · #9 리워드광고 parseInt · #10 코인 잔액 서버동기화 + 죽은 ctStoreExchange. + b571 어드민 가게핀 링크필터 반전.
>
> **실행 완료 SQL(사용자 확인):** `redeem_item_rpc.sql` · `price_pins_dedup.sql` · `phase2a_add_store_id.sql` · `phase2c_backfill_store_id.sql` · **`phase2e_drop_name_unique.sql`(비가역 실행됨)**.
>
> **#6 잔여(사소, 발생 시):** `store_photos.is_primary` UNIQUE 가 store_name 기준이면 동명 분리 후 충돌 가능.
> **다음 후보:** AdSense 승인 후 블로그 광고 슬롯 / 몰·빌딩 층 구분([`NEXT_TASK_mall_floor.md`](NEXT_TASK_mall_floor.md)) / 商品券 현금화 붙일 때 이메일확인 ON + abuse 방어.
...
### D. 자동추천 개선 + 코드 점검·정리 (b528~530)
- **b528** `_guessCtCategory`: 최초매칭 → **최장(=구체) 키워드 매칭**. 「とんかつ定食」=とんかつ>定食, 「スープカレー」=スープカレー>カレー. 누락 키워드 보강(うな重·うな丼 / 珈琲·ラテ).
- **b529** 코드 점검(리뷰 에이전트 2종: 버그/찌꺼기) — **버그 0건 확인**. 정리: 중복 i18n 키 `scan.camera.sub` 제거(구값 죽은키), 데드함수 `computeItemRankings`/`buildRankListHtml` 제거(호출처 0, 비활성 랭킹 잔재·한글 포함). `renderRanking` 스텁은 유지.
- **b530** 정적 중복 `ct.cat.*` 장르키(구 14개×4블록) 제거 — `_injectGenreI18n` 런타임 주입이 정본. `ct.cat.all`·`ct.cat.search_ph`(주입 안 됨)는 유지.

### 커밋
`d08fee0`(주차입력 b526) → `2e1f08c`(카테고리확장 b526) → `fb4d637`(필터통합 b527) → (b528 자동추천) → `b529 정리` → `b530 정리`. 모두 push.

---

## 0-K. 2026-06-16 — 4개국어 i18n 전면 적용 **완료** + 규칙 문서화 (build 516~525)

**배경:** 한국어 모드인데도 치리 공개 모달 등이 일본어로 노출. 전수 조사(Explore 3개 병렬) 결과 어드민 제외 **~245곳** i18n 미적용. **상세 목록·재현 방법은 `I18N_TODO.md`, 새 UI 만들 때 규칙은 `I18N_GUIDE.md`가 정본.**
...
| 518 | 커뮤니티 가격비교 패널 |
| 519 | 수동 핀 추가 모달 |
| 520 | 가게 수정/위치 요청 + 가게탭 힌트 |
| 521 | 지도 화면(치즈/치리토크/리워드) |
| 522 | 설정 + 추천(레퍼럴) + 온보딩 |
| 523 | 홈 추천배너 + 이력/리포트 |
| 524 | 출석체크/광고시청 페이지 + 출석 렌더 |
| 525 | 광고 시청 완료 화면·토스트(11분기) |

각 영역 신규 키를 `var I18N`(~13448)의 ja/ko/en/zh **4블록 모두**에 추가 + (키 4개 카운트 + 문법) 검증 후 영역별 커밋.

**남은 잔여(저빈도, 메인 화면 영향 적음):** 코인 내역 라벨(`ctAddCoin` 일본어 → `coinlog.*` 일괄), `renderPriceComparison`(가격비교 탭) 미조사, 업로드 확인 모달 엣지, GPT설정(개발용 추정). → `I18N_TODO.md` "남은 잔여 그룹".

**🌐 규칙 문서화(b a9ca4d9):** **`I18N_GUIDE.md`** 신설 — 새 페이지/카드/모달/토스트 등 **모든 사용자 대면 UI는 처음부터 4개국어**. 키를 `I18N` ja블록에 넣으면 **어드민 "언어시트"(i18n CMS)에 자동 노출**(목록은 `Object.keys(I18N.ja)` 기반 ~20154, override는 `i18n_translations` 테이블 Realtime). `AGENTS.md`에 연결해 **세션마다 자동 적용**.

**핵심 메커니즘:** 정적 `data-i18n`/`data-i18n-placeholder`/`data-i18n-title`(applyLang ~14600), 동적 `t('key',{n})`(~14547). 카테고리 라벨 헬퍼 `_cpCatLabel()`. **자식요소 있는 컨테이너엔 data-i18n 직접 X → 텍스트만 span.** 커밋 전 문법검사+키 4블록 카운트+빌드번호 2곳.
