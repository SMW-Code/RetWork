...
## ⭐ 업데이트: 그루메 블로그는 이제 직접 글쓰기 페이지가 있음

**그루메 블로그(blog.retwork.jp)** 에 비개발자용 작성 페이지를 만들었어요:
- 주소: **`blog.retwork.jp/admin`** (관리자 비번 입력 → 글쓰기)
- 기능: 새 글 작성 · 글 목록 · 수정 · 삭제 · 이미지 업로드 · **SEO 자동**
- 글 쓰고 "발행" 누르면 → GitHub 자동 커밋 → 블로그 반영 (Git/터미널 불필요)
- 상세 문서: `retwork-blog` 레포의 **`BLOG_EDITOR.md`**
- ⏳ 남은 일: 실제 발행 end-to-end 테스트 (BLOG_EDITOR.md 2번 체크리스트)

> 절약 블로그(retwork.jp/blog)는 아직 손HTML → 새 글은 Claude한테 요청하거나, 나중에 같은 에디터 적용 가능.
